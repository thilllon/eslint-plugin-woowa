...
export const hasMismatchedInjected = (node: TSESTree.VariableDeclarator): boolean => {
  ...
  const injectedParameter = nestProviderAstParser.findProvideProperty(node, 'inject')
    ?.value as unknown as TSESTree.ArrayExpression;

  const injectedParameterCount = injectedParameter ? injectedParameter.elements.length : 0;
...
const rule = createRule({
  name: 'nestjs-provided-injected-should-match-factory-parameters',
  meta: {
    docs: {
      description:
