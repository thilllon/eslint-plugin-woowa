...
export const checkObjectExpression = (os: TSESTree.ObjectExpression): boolean => {
  if (!os) {
    return false;
  }
  const forbidUnknownValuesProperty = os?.properties?.find(
    (p) => ((p as TSESTree.Property).key as TSESTree.Identifier).name === 'forbidUnknownValues'
...
export const shouldTriggerNewExpressionHasProperty = (node: TSESTree.Node): boolean => {
...
export const shouldTriggerForVariableDecleratorExpression = (node: TSESTree.Node): boolean => {
  ...
  const typeAnnotation = asExpression?.typeAnnotation as TSESTree.TSTypeReference;
  ...
  return checkObjectExpression(asExpression.expression as TSESTree.ObjectExpression);
};

const rule = createRule({
  name: 'nestjs-validation-pipe-should-use-forbid-unknown',
  meta: {
    docs: {
      description:
  ...
  create(context: Readonly<TSESLint.RuleContext<'shouldSpecifyForbidUnknownValues', never[]>>) {
    return {
      ...
      VariableDeclarator(node: TSESTree.Node): void {
        // eslint-disable-next-line @typescript-eslint/naming-convention
        const result = shouldTriggerForVariableDecleratorExpression(node);
