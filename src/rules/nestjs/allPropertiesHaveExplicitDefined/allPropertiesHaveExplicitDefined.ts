...
const rule = createRule({
  name: 'nestjs-all-properties-have-explicit-defined',
  meta: {
    docs: {
    ...
    return {
      ...
      ClassDeclaration(node) {
        ...
        for (const propertyDefinition of propertyDefinitions) {
          ...
          propertyDefinitionsWithDecoratorsStatus.push([propertyDefinition, decoratorsStatus]);

          // It doesn't make sense to have both @IsDefined and @IsOptional decorators
          if (decoratorsStatus.hasIsDefinedDecorator && decoratorsStatus.hasIsOptionalDecorator) {
            context.report({
        ...
        if (withDecoratorCount > 0) {
          for (const [
            ...
            if (propertyDefinition.optional || isNullableType(type)) {
...
function getDecoratorsStatus(propertyDefinition: TSESTree.PropertyDefinition): DecoratorsStatus {
