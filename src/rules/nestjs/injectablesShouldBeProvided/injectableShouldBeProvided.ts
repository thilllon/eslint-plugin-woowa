...
const checkNode = (
  ...
  context: Readonly<TSESLint.RuleContext<'injectableInModule' | 'controllersInModule', Options>>
) => {
...
function initialiseModuleMappings(
  ...
  const mappedSourceDirectory = NestProvidedInjectableMapper.detectDirectoryToScanForFiles(
    sourcePath,
    process.cwd()
  );
...
const rule = createRule({
  name: 'nestjs-injectable-should-be-provided',
  meta: {
    docs: {
  ...
  create(
    ...
    return {
      ...
      'Program:exit'(): void {
        ...
        const mappedProvidedInjectables = NestProvidedInjectableMapper.mapAllProvidedInjectables(
