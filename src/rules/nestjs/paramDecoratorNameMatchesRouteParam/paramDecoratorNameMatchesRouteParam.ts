...
export const parsePathParts = (decorator: TSESTree.Decorator): string[] => {
  const decoratorArgument = (decorator?.expression as TSESTree.CallExpression)?.arguments[0];

  if (decoratorArgument?.type === 'Literal') {
    return [decoratorArgument.raw];
  }
  if (decoratorArgument?.type === 'ArrayExpression') {
    return decoratorArgument.elements.map((x) => (x as TSESTree.Literal).raw);
  }
  if (decoratorArgument?.type === 'ObjectExpression') {
    return decoratorArgument.properties
      .filter((x) => ((x as TSESTree.Property)?.key as TSESTree.Identifier)?.name === 'path')
      .map((x) => ((x as TSESTree.Property).value as TSESTree.Literal).raw);
  }
  return [];
};
 ...
 */
export const hasPathPartsAnyRegexParams = (pathPartsToCheck: string[]): boolean => {
  // prettier-ignore
...
export const shouldTrigger = (decorator: TSESTree.Decorator): ResultModel => {
  ...
  // grab the param name
  const paramName = (
    (decorator.expression as TSESTree.CallExpression)?.arguments[0] as TSESTree.Literal
  )?.value as string;
  ...
  const controllerDecorator = (
    decorator.parent?.parent?.parent?.parent?.parent as TSESTree.ClassDeclaration
  )?.decorators?.find((d) => {
    return (
  ...
  const methodDefinition = decorator.parent?.parent?.parent as TSESTree.MethodDefinition;
  ...
  const shouldIgnoreThisSetOfRoutes = hasPathPartsAnyRegexParams(pathPartsToCheck);
...
const rule = createRule({
  name: 'nestjs-param-decorator-name-matches-route-param',
  meta: {
    docs: {
      description:
  ...
  create(
    context: Readonly<
      TSESLint.RuleContext<'paramIdentifierDoesntNeedColon' | 'paramIdentifierShouldMatch', never[]>
    >
  ) {
    return {
      ...
      Decorator(node: TSESTree.Decorator): void {
        if (
          ((node.expression as TSESTree.CallExpression)?.callee as TSESTree.Identifier)?.name !==
          'Param'
        ) {
          return;
        }
