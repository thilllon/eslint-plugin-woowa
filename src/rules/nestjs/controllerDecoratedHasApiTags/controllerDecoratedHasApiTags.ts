...
export const shouldUseApiTagDecorator = (node: TSESTree.ClassDeclaration): boolean => {
  const hasControllerDecorator = typedTokenHelpers.nodeHasDecoratorsNamed(node, ['Controller']);

  const hasApiTagDecorator = typedTokenHelpers.nodeHasDecoratorsNamed(node, ['ApiTags']);

  return hasControllerDecorator && !hasApiTagDecorator;
};

const rule = createRule({
  name: 'nestjs-controllers-should-supply-api-tags',
  meta: {
    docs: {
  ...
  create(context: Readonly<TSESLint.RuleContext<'shouldUseApiTagDecorator', never[]>>) {
    return {
