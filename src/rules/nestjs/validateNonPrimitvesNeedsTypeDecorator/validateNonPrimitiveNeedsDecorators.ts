...
import { AST_NODE_TYPES, TSESTree, ESLintUtils, TSESLint } from '@typescript-eslint/utils';
...
const rule = createRule({
  name: 'nestjs-validated-non-primitive-property-needs-type-decorator',
  meta: {
    docs: {
  ...
  create(
    ...
    const { additionalTypeDecorators, additionalCustomValidatorDecorators } = context
      .options[0] || {
      additionalTypeDecorators: [],
    };
    ...
    return {
      ...
      PropertyDefinition(node: TSESTree.PropertyDefinition): void {
        ...
        if (isAnArray) {
          const mainTypeInShortArray = (node.typeAnnotation?.typeAnnotation as TSESTree.TSArrayType)
            ?.elementType?.type;
          if (mainTypeInShortArray) {
            mainType = mainTypeInShortArray;
          } else {
            // try to get the type of Array<type> syntax
            const foundParams = (node.typeAnnotation?.typeAnnotation as TSESTree.TSTypeReference)
        ...
        const isNodeAUnionWithAPrimitive =
          ...
          (node.typeAnnotation?.typeAnnotation as TSESTree.TSUnionType).types?.some((x) =>
        ...
        const foundClassValidatorDecorators = typedTokenHelpers.getValidationDecorators(
          node,
          additionalCustomValidatorDecorators
        );
        ...
        const hasObject = foundClassValidatorDecorators.some((foundClassValidatorDecorator) =>
          typedTokenHelpers.decoratorIsIsObject(foundClassValidatorDecorator)
        );
          ...
          if (typeAnnotationNode) {
            if (
              ...
              if (typeAnnotationNode.typeName.name === 'Array') {
                ...
                if (foundParams && foundParams.length === 1) {
                  const typeName = (foundParams[0] as TSESTree.TSTypeReference).typeName;
            ...
            } else if (typeAnnotationNode.type === AST_NODE_TYPES.TSArrayType) {
              const elementTypeNode = typeAnnotationNode.elementType;

              if (
                elementTypeNode.type === AST_NODE_TYPES.TSTypeReference &&
                elementTypeNode.typeName.type === AST_NODE_TYPES.Identifier
              ) {
