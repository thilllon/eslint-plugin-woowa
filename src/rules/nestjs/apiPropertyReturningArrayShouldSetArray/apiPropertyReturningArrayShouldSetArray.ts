...
export const shouldSetArrayProperty = (node: TSESTree.PropertyDefinition): ArraySetResultModel => {
  ...
  const firstArgumentToDecorator = (decorators[0].expression as TSESTree.CallExpression)
    .arguments[0] as TSESTree.ObjectExpression;

  const hasIsArraySetInOptions = typedTokenHelpers.getPropertyValueEqualsExpected(
  ...
  const isArrayType =
    ((typeAnnotation as TSESTree.TSTypeReference | undefined)?.typeName as TSESTree.Identifier)
      ?.name === 'Array';
...
const rule = createRule({
  name: 'nestjs-api-property-returning-array-should-set-array',
  meta: {
    docs: {
  ...
  create(
    context: Readonly<
      TSESLint.RuleContext<'shouldSetArrayPropertyTrue' | 'shouldSetArrayPropertyFalse', never[]>
    >
  ) {
    return {
      // eslint-disable-next-line @typescript-eslint/naming-convention
      PropertyDefinition: (node: TSESTree.Node) => {
        const shouldSetArrayResults = shouldSetArrayProperty(node as TSESTree.PropertyDefinition);
