...
export const hasEnumSpecifiedCorrectly = (
  ...
  const decorators = typedTokenHelpers.getDecoratorsNamed(node as TSESTree.PropertyDefinition, [
    'ApiPropertyOptional',
    'ApiProperty',
  ]);
  ...
  const enumProperty = firstArgument.properties.find(
    (p) => ((p as TSESTree.Property).key as TSESTree.Identifier).name === 'enum'
  );
  ...
  const hasTypeProperty =
    firstArgument.properties.find(
      (p) => ((p as TSESTree.Property).key as TSESTree.Identifier).name === 'type'
    ) !== undefined;
  ...
  const enumNameProperty = firstArgument.properties.find(
    (p) => ((p as TSESTree.Property).key as TSESTree.Identifier).name === 'enumName'
  );
  return new EnumTestResultModel({
...
const rule = createRule({
  name: 'nestjs-api-enum-property-best-practices',
  meta: {
    docs: {
