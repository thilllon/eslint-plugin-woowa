...
export const shouldSetArrayProperty = (node: TSESTree.PropertyDefinition): ArraySetResultModel => {
  const decorators = typedTokenHelpers.getDecoratorsNamed(node, ['ValidateNested']);
  ...
  const hasEachSetInOptions = typedTokenHelpers.getPropertyValueEqualsExpected(
...
const rule = createRule({
  name: 'nestjs-validate-nested-of-array-should-set-each',
  meta: {
    docs: {
      description:
  ...
  create(
    context: Readonly<
      TSESLint.RuleContext<'shouldSetEachPropertyTrue' | 'shouldSetEachPropertyFalse', never[]>
    >
  ) {
    return {
