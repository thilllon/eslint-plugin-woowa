...
describe('nest module ast mapper', () => {
  it('Can parse typescript typings using the typescript parser', () => {
    ...
    const typeAnnotation = (result.body[0] as TSESTree.VariableDeclaration).declarations[0].id
      .typeAnnotation;
  ...
  test.each([moduleMappingTestData])(
    ...
    (testItem: {
      ...
      const modules = nestModuleAstMapper.mapAllProvidedInjectables(ast, fakeFilePath);
