...
export const nestProviderAstParser = {
  mapNestProviderObject(
    ...
    if (propertyName) {
      return [path, new NestProvidedInjectablesMap(new Set(), new Set([propertyName]))];
    }
    return null;
  },
  findProvideProperty(
    ...
    if (providerDeclaration) {
      const foundProviderProperty = (
        ...
        (p) =>
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ((p as TSESTree.Property).key as TSESTree.Identifier).name === propertyName
      ...
      if (
        ...
        const providerDeclaration = n.declaration.declarations.find((d) => {
          ...
          const hasProvideProperty: boolean =
            isObjectExpression &&
            (d.init as TSESTree.ObjectExpression).properties.some((property) => {
              return (
