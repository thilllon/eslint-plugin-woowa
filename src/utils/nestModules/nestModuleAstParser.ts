...
export const nestModuleAstParser = {
  findNestModuleClass(ast: TSESTree.Program): TSESTree.ClassDeclaration | null {
    for (const n of ast.body) {
      ...
      if (n.type === AST_NODE_TYPES.ClassDeclaration && n.decorators && n.decorators.length > 0) {
        return n;
      }
  ...
  mapNestModuleDecorator(
    ...
    const moduleDecorator = n.decorators?.find(
      (d) =>
        ((d.expression as TSESTree.CallExpression).callee as TSESTree.Identifier)?.name === 'Module'
    );
    if (moduleDecorator) {
      const mappedControllerElements = this.mapModuleDecoratorOptionProperty(
        moduleDecorator,
        'controllers'
      );
      const mappedProviderElements = this.mapModuleDecoratorOptionProperty(
        moduleDecorator,
        'providers'
      );
  ...
  mapModuleDecoratorOptionProperty(
    ...
    if (optionProperty) {
      ...
      const propertyAsArrayExpressionElements =
        ((optionProperty as unknown as TSESTree.Property).value as TSESTree.ArrayExpression)
          ?.elements || [];

      return new Set(
        propertyAsArrayExpressionElements.map((element) => (element as TSESTree.Identifier).name)
      );
    }
