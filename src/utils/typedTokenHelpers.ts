...
export const typedTokenHelpers = {
  ...
  isTypeArrayTypeOrUnionOfArrayTypes(
    ...
    if (
      (node as TSESTree.PropertyDefinition)?.typeAnnotation?.typeAnnotation?.type ===
      TSESTree.AST_NODE_TYPES.TSArrayType
    ) {
      return true;
    }
  ...
  getPropertyValueEqualsExpected(
    ...
    if (firstArgument !== undefined) {
      const foundPropertyOfName = firstArgument.properties.find(
        (p) => ((p as TSESTree.Property).key as TSESTree.Identifier).name === propertyName
      );

      didMatchExpectedValues =
        foundPropertyOfName !== undefined &&
        ((foundPropertyOfName as TSESTree.Property).value as TSESTree.Literal).value ===
          expectedValue;
    }
  ...
  nodeHasDecoratorsNamed(
    n: TSESTree.ClassDeclaration | TSESTree.PropertyDefinition | TSESTree.MethodDefinition,
  ...
  isEnumType(type: ts.Type) {
    ...
    if (isTypeFlagSet(type, ts.TypeFlags.Enum)) return true;
    if (isTypeFlagSet(type, ts.TypeFlags.EnumLike)) return true;

    // it's not an enum type if it's an enum literal type
    if (isTypeFlagSet(type, ts.TypeFlags.EnumLiteral) && !type.isUnion()) return false;
  ...
  isOptionalPropertyValue(node: TSESTree.PropertyDefinition): boolean {
    const isUndefinedType =
      (node.typeAnnotation?.typeAnnotation as TSESTree.TSUnionType)?.types?.find(
  ...
  getValidationDecorators(
    ...
    return (
      decorators?.filter((decorator): decorator is TSESTree.Decorator => {
        const isClassValidatorDecorator = typedTokenHelpers.decoratorIsClassValidatorDecorator(
          program,
          decorator
        );
  ...
  getDecoratorName(decorator: TSESTree.Decorator): string | null {
    ...
    if (decorator.expression.callee.type !== TSESTree.AST_NODE_TYPES.Identifier) {
      return null;
    }
